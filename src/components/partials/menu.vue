<template>
  <div>
    <v-navigation-drawer v-model="model" app>
      ...
      <v-list>
        <v-subheader>{{ $t("labels.experiences") }}</v-subheader>

        <v-list-item
          v-for="(item, i) in experiences"
          :key="i"
          @click="showItem(item, 'experiences')"
        >
          <v-list-item-icon>
            <v-icon>android</v-icon>
          </v-list-item-icon>
      ...
      <v-list>
        <v-subheader>{{ $t("labels.skills") }}</v-subheader>

        <v-list-item
          v-for="(item, i) in skills"
          :key="i"
          @click="showItem(item, 'skills')"
        >
          <v-list-item-icon>
            <v-icon>android</v-icon>
          </v-list-item-icon>
    ...
    <v-app-bar class="primary" app>
      ...
      <v-toolbar-title>
        <v-btn class="secondary" @click="checkNameRoute(visit_list[0])"
          >Jesús Martínez</v-btn
        >
      </v-toolbar-title>
      <v-progress-linear
...
export default {
  watch: {
    ...
    theme_on() {
      ...
      document.getElementById("body").style.backgroundColor = this.theme_on
        ? "#FFF"
        : "#303030";
    },
    experiences() {
